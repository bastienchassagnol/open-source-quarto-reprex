...
authors:
  - name: Justin Belair
    id: jb
    orcid: 0000-0002-4219-1824
    email: belairjustin@gmail.com
    url: https://justinbelair.com/
    corresponding: true
    phone:  1 (514) 755-2851
    affiliation:
      - name: McGill
        city: Montreal
        state: Quebec
  ...
  - name: Bastien CHASSAGNOL
    ...
    url: https://github.com/bastienchassagnol
    roles:
      - analyis: supporting
    ...
    degrees:
      ...
      - PhD
    affiliation:
      - name: Marseille Medical Genetics
        department: Systems Biomedicine
        city: Marseille
        country: France
        url: https://www.marseille-medical-genetics.org/a-baudot/
description: |
  A comprehensive review of practical
  causal use cases, accompagnied with R snippets.
abstract: |
  That is, bring you, dear reader, up-to-date on current statistical methodology
  in causal inference! I made the book self-contained, so we will go over
  all the necessary theory.
keywords:
  - Causal Analysis
  - DAGs
  - Statistics
license: "CC BY-NC-ND" # The most restrictive one
copyright:
  holder: Justin Belair
  year: 2024 - 2030
# To make it citeable
funding: "The author received no specific funding for this work."
lang: en-GB
subject: "Causal Analysis"
citation: true # Returns with the URL of the website
google-scholar: true
# Format genetation
...
freeze: true
# code options
code-block-border-left: "#6A6A68"   # dark grey border
code-block-bg: "#F7F6F2"            # light grey background
filters:
  - highlight-text
  - callouty-theorem
  - first-line-indent
...
format:
  ...
  pdf: 
    documentclass: book
    fontfamily: tgpagella # only required for pdflatex
...
First derivation of drug fingerprints using innovative cellular barcoding applied to cancer cells subclones
...
::: {.proof}
A full proof can be found in [@pearl_2009 p.80]. The gist of it is that, when $Z$ blocks all backdoor paths, $\mathbb{P}(Y=y|do(W=w),Z=z) = \mathbb{P}(Y=y|W=w, Z=z).$
...
(Exceptional Situations for *d*-separation) Consider the simplest two-variable DAG, $X \rightarrow Y$, and denote $Z = \emptyset$. Suppose $X$ is a standard gaussian distribution with mean 0, and
$$
Y = \begin{cases}
...
## Maths commands with non-standard operators
...
-   The theoretical foundations of Directed Acyclic Graphs (DAGs), Bayesian Causal Networks (BCN), and Structural Causal Models (SCM)
-   The notion of Markov compatibility between a DAG and (families of) probability distributions
-   The notion of $d$-separation and how to use it to read conditional independencies from DAGs
-   The backdoor criterion and backdoor adjustment formula
-   Common DAG structures like confounding, colliders, mediators, M-bias, and butterfly bias
-   The definition of the $do$-operator as a mathematical model for experimental interventions, in both CBNs and SCMs, along with the truncated factorization formula

You should walk away from this chapter with the following skills:

-   A reliable understanding of DAG terminology and notation
-   A robust intuition about confounding, collider bias, mediation effects, etc.
-   A solid capability of identifying paths in DAGs and using them to determine identifiability of causal effects and conditional independencies in data
-   An advanced capacity to work through real-world examples by building different DAG structures and critically assessing the assumptions they imply
-   Intimate knowledge of R code for simulating and analyzing data based on DAG structures
...
This book stands on the shoulders of many great textbooks on causal
inference.

A very gentle, short, and captivating non-technical book is
[@rosenbaum2023causal]. The famous [@pearl2018book] book is a compelling
narrative view of the history of causal inference, yet it only covers
the point of view of Pearl’s camp. As we will see, there are many
different schools of thought in causal inference. A good introductory
book which eases into technical material is [@pearl_et_al_2016], yet it
has some flaws in my opinion. Indeed, it does not go too far into the
subject, as it is simply a primer and takes around a third of the space
to go over basic probability theory and graph theory before really
diving into causal inference. This short book thus leaves very little
space to go deeper in causal inference.

My aim has been to take the next step from this book. That is, bring
you, dear reader, up-to-date on current statistical methodology in
causal inference! I made the book self-contained, so we will go over all
the necessary theory, but we assume you, dear reader, is familiar with
probability and statistics and leave all this basic theory in the
@sec-appendix for your reference.

I attribute [@pearl_2009] with sparking my curiosity to go deeper in the
fascinating subject of causal inference–it’s a true masterpiece and I
recall fondly the intellectual awakening it sparked in me when I read it
for the first time. It is a difficult book written with the goal of
rigorously defining Pearl’s approach to causal inference, which relies a
lot on advanced mathematics–at least, more advanced than what a
traditional applied (data) scientist is equipped to handle.
[@imbens_rubin_2021] is another very elegant textbook which I loved
reading, but it is also heavy on mathematics. [@Morgan_Winship_2014] is
rich with references to literature and real-world examples taken from
decades of causal debates in social sciences, but it can be a bit heavy.
[@shadish2002experimental] is an amazing book on randomized and
quasi-experimental design, but it is more methodological and contains
virtually no mathematics. We will also discuss epidemiology as it has
become increasingly technical, with much overlap with what was
traditionally called biostatistics. The best book for causal inference
for epidemiologists is in my opinion [@hernan2024causal]. Plus,
epidemiology relies almost uniquely on observational data, where causal
inference methodology is indispensable. As we will see, a major
intellectual breakthrough in causal inference comes from our
understanding of when and how we can analyze observational data as if it
was experimental, so-to-speak. Great references for epidemiology and
more specifically causal inference within the field are [@ModernEpi] and
[@Westreich_2020]. There are even more specialized books like
[@vanderWeele2015] and [@Shipley_2000], an ecology book, that will be
referenced throughout. Economists have also written amazing causal
inference textbooks, such as [@cunningham_mixtape] and
[@angrist_mostly_2008]. Machine learning (ML) and Artificial
Intelligence (AI) based approaches to causal inference are introduced in
[@molak2023causal], which also serves as a great introduction to causal
discovery methods and uses Python, which is rather rare. ML and AI
approaches are covered in depth in [@chernozhukov2024applied]. An
interesting framework called Targeted Maximum Likelihood Estimation
(TMLE) is covered in depth in [@vanderLaan2011targeted]. All this is
without mentioning the 100s of papers I’ve read over the years that you
will find in the bibliography. This sprawling literature on causal
inference serves as a foundation to the book you’re about to read. So
many methods are published everyday and we cannot hope to cover all of
them–each Chapter of this book could receive a book-sized–or even a
multi-volume series length–coverage. The hope is that by learning
fundamentals, going through exercises, and practicing with code, a solid
basis is created upon which learning the future specialized methods you
may need need can be done without too much difficulty. Indeed, in my
opinion, mastering the material of this book would put you ahead of the
vast majority of causal inference practitioners,  **below, by jumping two lines, instead of one.**, and you.
<br> <br>

As a trained mathematician and statistician who’s taught dozens of
university-level courses to business students, data scientists,
engineers, and researchers, I have the necessary background to decipher
these complex textbooks and bring out the essence for
non-mathematicians, without having to go through long and rigorous
mathematical proofs. I’ve also worked as a statistical consultant with
dozens of scientists from around the world in many biomedical sciences,
as well as researchers in psychology, psychiatry, agronomy,
epidemiology, and even Research and Development (R&D) in biotechnology
and pharmaceutical industries. I’ve been applying causal inference in
applied scientific research for a long time now, through my services
offered at a company I founded and run on a daily basis, JB Statistical
Consulting[^index-1]. I’m also a philosophy dilettante, who’s been
mostly interested in figuring out how this beautiful thing we call
Science actually works and how to do it well, through reading philosophy
of science, history of science, epistemology, and more. All this is to
say that I (again, very humbly) believe I’m uniquely positioned to take
the complex mathematical and philosophical ideas behind causal
inference, decipher them and bring them to a level where an applied
scientist can use them. Plus, I absolutely love sharing knowledge I’m
passionate about!


## Mailto example

- Instead of `\email`, use `[belairjustin@gmail.com](mailto:belairjustin@gmail.com)`.

## Toc example {.unnumbered .unlisted}

### Unumbered header {.unnumbered}

### Not present in the toc {.unlisted}
...
## Paragraph format, and large block of texts
...
$$
 although this notation is quite heavy. We oftentimes simply resort to picturing the graph. Indeed a pictorial representation of this graph. This graph is directed and acyclic, thus it is a DAG and can be used for causal inference. We will learn how to do soon.
...
## Tables generation with `flextable` and `gt`

- Example with `flextable` in @tbl-two-confounder-DAG-coef-flextable. Not recommended as this does not integrate with `quarto` and a `pdflatex` rendering:

```{r}
#| label: tbl-two-confounder-DAG-coef-flextable
...
df_two_confounder <- rbind(Y_W_coef, Y_W_Z1_coef, Y_W_Z1_Z2_coef) |>
  round(2) |>
  tibble::as_tibble() |> # only tibble preserves colnames as such
  rename("Z_1" = V3, "Z_2" = V4) |>
  rownames_to_column("Model") |>
  select(-`(Intercept)`) |>
  mutate(Model = c("Y \\sim W", "Y \\sim W + Z_1", "Y \\sim W + Z_1 + Z_2"))

ft <- flextable(df_two_confounder) |>
  compose(j = "Model", part = "body",
          value = as_paragraph(as_equation(`Model`, width = 2, height = .5))) |>
  compose(j = "W", part = "header",
          value = as_paragraph(as_equation(`W`, width = 2, height = .5))) |>
  compose(j = "Z_1", part = "header",
          value = as_paragraph(as_equation(`Z_1`, width = 2, height = .5))) |>
  compose(j = "Z_2", part = "header",
          value = as_paragraph(as_equation(`Z_2`, width = 2, height = .5))) |>
...
- Example using the `gt` package paired with `gt::fmt_markdown()` function in @tbl-two-confounder-DAG-coef-gt:

```{r}
#| label: tbl-two-confounder-DAG-coef-gt
#| tbl-cap: "Model coefficients for data generated from the two-confounder DAG"
...
# Build gt table
gt_tbl <- df_two_confounder |>
  gt(rowname_col = "Model") |>
  gt::fmt_markdown() |>
  gt::cols_label(W ~ gt::md("$W$"), 
                 Z_1 ~ gt::md("$Z_1$"), 
                 Z_2 ~ gt::md("$Z_2$")) |> 
  gt::sub_missing(columns = dplyr::everything(), missing_text = "") |> # deal with missing columns
  tab_style(
